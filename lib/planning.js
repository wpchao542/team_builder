...
function normalizeDecisionAliases(decision) {
  ...
  // 有的模型把动作字段包进与 action 同名（或 payload/params）的子对象里，例如
  //   { action:"dispatch", dispatch:{ member_id, instruction } }
  // 这里先把这种嵌套摊平到顶层，再做字段别名归一，避免误判成“member_id 为空”。
  const isPlainObj = (v) => v && typeof v === "object" && !Array.isArray(v);
  for (const wk of [decision.action, "payload", "params", "args", "decision"].filter(Boolean)) {
    const sub = decision[wk];
    if (Array.isArray(sub)) {
      if (decision.action === "dispatch_parallel" && !(Array.isArray(decision.parallel_calls) && decision.parallel_calls.length)) {
        decision.parallel_calls = sub; // { action:"dispatch_parallel", dispatch_parallel:[...] }
      }
      continue;
    }
    if (isPlainObj(sub)) {
      for (const [k, v] of Object.entries(sub)) {
        ...
        if (cur == null || cur === "" || (Array.isArray(cur) && cur.length === 0)) decision[k] = v;
      }
    }
  }
...
function updateTeamTool(spec, applyTeamEvolution) {
  const ids = (spec.agents || []).map((a) => a.id);
  return {
    name: HARNESS_UPDATE_TEAM_TOOL,
    description: "让团队【长期进化】（不是一次性产出）。当用户的反馈意味着团队的做法/分工要固化改变时调用：可改成员的执行契约(member_edits)、新建成员(new_members)、改团队目标(summary)、追加全员遵守的全局补充规则(note)。系统会自动重建全局 Skill 并保存团队。【一致性检查·必做】决定 update_team 前，先通盘检查这次改动会牵连到谁——一个变更往往不止影响一个成员：上下游接口/字段/命名/格式变了，相关成员（供料方、消费方、共享同一约定的成员）也要一并更新；若是全员适用的规则，写进 note 让全队遵守，而不是只改一个成员、留下别处不一致。宁可一次把受影响的成员都在 member_edits 里改齐，避免改一半导致团队前后矛盾。改完后你仍要正常提交本轮调度决策（通常接着派受影响的成员去重做）。一次性的小调整不要用它。",
    schema: {
      type: "object",
      additionalProperties: false,
      properties: {
    ...
    run: (args, toolCtx) => {
      const evolve = applyTeamEvolution || toolCtx.applyTeamEvolution;
      if (typeof evolve !== "function") return { ok: false, content: "团队进化能力未注入（applyTeamEvolution 缺失）。" };
      const r = evolve(toolCtx.spec, { by: "将军", member_edits: args.member_edits, new_members: args.new_members, summary: args.summary, note: args.note });
