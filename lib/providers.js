...
function claudeCodeOnce(system, user, modelArg, onThinking, signal) {
  return new Promise((resolve, reject) => {
    ...
    // 停战/打断：杀掉 claude 子进程并让 await 立即解开（否则停战卡住，成员一直在干）
    let killTimer = null;
    const onAbort = () => {
      try { cp.kill("SIGTERM"); } catch {}
      killTimer = setTimeout(() => { try { cp.kill("SIGKILL"); } catch {} }, 1500);
      if (!settled) { settled = true; reject(new Error("[已停战] Claude Code 调用被中断")); }
    };
    const cleanup = () => { if (killTimer) clearTimeout(killTimer); if (signal) { try { signal.removeEventListener("abort", onAbort); } catch {} } };
    if (signal) {
    ...
    cp.on("error", (e) => { cleanup(); if (settled) return; settled = true; reject(new Error(`无法启动 Claude Code CLI：${e.message}`)); });
    cp.on("close", (code) => {
      cleanup();
      if (settled) return;
      settled = true;
...
function codexExecOnce({
  ...
  return new Promise((resolve, reject) => {
    ...
    // 停战/打断：杀掉 codex 子进程并让 await 立即解开
    let killTimer = null;
    const onAbort = () => {
      try { cp.kill("SIGTERM"); } catch {}
      killTimer = setTimeout(() => { try { cp.kill("SIGKILL"); } catch {} }, 1500);
      if (!settled) { settled = true; if (schemaDir) { try { fs.rmSync(schemaDir, { recursive: true, force: true }); } catch {} } reject(new Error("[已停战] Codex 调用被中断")); }
    };
    const cleanupAbort = () => { if (killTimer) clearTimeout(killTimer); if (signal) { try { signal.removeEventListener("abort", onAbort); } catch {} } };
    if (signal) {
      if (signal.aborted) { onAbort(); return; }
      signal.addEventListener("abort", onAbort, { once: true });
    }
...
// 用 process.env（loadConfig 注入 config.json 之后）重新派生所有 provider 运行时配置。
// 必须在 server.js loadConfig() 之后调用：本模块在 require 时即按当时的 env 初始化了上述 let，
// 而 loadConfig() 通常晚于 require，故那批默认值要在这里用配置后的 env 校正，否则 config.json 失效。
