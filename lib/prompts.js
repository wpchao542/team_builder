...
9. 团队基础配置（base_config）：把会被【2 个或以上成员】共用的非密钥变量提取到 base_config，做全队统一取值（如目标分辨率、配音音色、输出命名前缀、目标平台、风格 tokens、公共路径等），每条给 key/value/desc；value 当下不确定可留空。只被单个成员用的变量不要放，密钥放 secrets 不放这里。没有共用变量就给空数组。`;
...
const TEAM_SCHEMA = {
  ...
  properties: {
    ...
    base_config: {
      type: "array",
      description: "团队基础配置：被【2 个或以上成员】共用的非密钥变量，提取到这里做全队统一取值（如目标分辨率、配音音色 id、输出文件命名前缀、目标平台、风格 tokens、公共路径等）。只被单个成员用的不要放这里。运行时会以同名环境变量注入，shell 里可用 $KEY 取；也会写进团队全局 Skill 让全员统一。没有就留空数组。",
      items: {
        type: "object", additionalProperties: false, required: ["key", "value", "desc"],
        properties: {
          key: { type: "string", description: "变量名（同时作为环境变量名），如 TARGET_RESOLUTION、VOICE_ID" },
          value: { type: "string", description: "取值；点将时若还不确定可留空字符串等用户/运行时填" },
          desc: { type: "string", description: "一句话说明它是什么、哪些成员会用到" },
        },
      },
    },
  },
...
10. 团队基础配置（base_config）：把蓝图里会被【2 个或以上成员】共用的非密钥变量提取到 base_config 做全队统一取值（如目标分辨率、配音音色、输出命名前缀、目标平台、风格 tokens、公共路径等），每条给 key/value/desc；用户已拍板的取值直接填进 value，未定可留空。只被单个成员用的不要放，密钥放 secrets。没有就给空数组。`;
...
12. 原文忠实性指的是【skill 的命令、参数、顺序、模板、判断条件、产物要求】不得改写、删减或“优化”。但用户在输入框里的额外说明要【认真采纳】，用它来决定团队怎么搭：成员的命名、分工与边界、各成员的侧重与职责描述、哪些模块合并给同一成员或如何分配，都应体现用户的意图。简言之——照用户的话塑造团队结构，但不动 skill 的执行配方。若用户的说明确实要改变 skill 的执行行为（与原文冲突），以原文为准，并可在 summary 里说明。

13. 团队基础配置（base_config）：把 skill 原文里被【2 个或以上成员/模块】共用的非密钥变量提取到 base_config 做全队统一取值（如分辨率、音色、命名前缀、公共路径、风格 tokens 等），每条 key/value/desc，value 取原文里的实际值、没有就留空；只被单个模块用的不要放，密钥仍放 secrets。这是为了全队取值一致，不改变原 skill 的任何步骤。没有就给空数组。
