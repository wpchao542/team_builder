...
> 后端正在从单文件渐进式拆分为 `lib/` 模块：先抽出零运行时依赖的「纯数据 / 纯函数 / 持久化」层（提示词、schema、归一逻辑、纯决策校验、运行存储），运行时核心（providers / harness / orchestrator / 路由）仍在 `server.js`，后续按同样的「抽一块、跑一遍测试」方式继续拆。前端也已从单文件 `index.html` 拆成 结构 / 样式 / 逻辑 三个文件。
